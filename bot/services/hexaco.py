...
STATEMENTS: List[Statement] = [
    ...
    Statement(
        25,
        "Как часто вы заранее продумываете, какие выгоды можно получить от общения с человеком?",
    ),
    Statement(
        26,
        "Насколько вы избегаете использовать чужие слабости даже ради важной цели?",
    ),
    Statement(
        27,
        "Как часто вы чувствуете, что достойны особого отношения по сравнению с окружающими?",
    ),
    Statement(
        28,
        "Насколько сильно вы расстраиваетесь, если ваши успехи не отмечают коллеги/руководители?",
    ),
    Statement(
        29,
        "Как часто вы действуете импульсивно, даже если это может навредить планам?",
    ),
    Statement(
        30,
        "Насколько вас останавливает возможный ущерб другим людям при принятии рискованных решений?",
    ),
]
...
BANDS_TRIAD: Sequence[Band] = [
    Band("low", "Низкий", 0, 33.33, min_inclusive=True),
    Band("medium", "Средний", 33.33, 66.67),
    Band("high", "Высокий", 66.67, 100, max_inclusive=True),
]
...
def _interp_triad(levels: List[str]) -> Dict[str, str]:
    band_ids = [band.id for band in BANDS_TRIAD]
    if len(levels) != len(band_ids):
        raise ValueError("Interpretations count does not match triad band count.")
    return dict(zip(band_ids, levels, strict=True))
...
DOMAINS: Sequence[DomainDefinition] = [
    ...
    DomainDefinition(
        id="dt_machiavellianism",
        title="Макиавеллизм",
        visibility="internal",
        items=[DomainItem(25), DomainItem(26, True)],
        interpretations=_interp_triad(
            [
                "Низкий уровень склонности к манипуляциям и прагматичному использованию людей. Обычно действует прямо, избегает эксплуатации слабостей других.",
                "Умеренная склонность к расчёту и выгоде. Может сочетать прагматизм с соблюдением норм, выбирая подход по ситуации.",
                "Высокая склонность к инструментальному использованию людей и поиску выгод. Выше риск манипулятивных ходов и обхода правил ради цели.",
            ]
        ),
    ),
    DomainDefinition(
        ...
        title="Нарциссизм",
        visibility="internal",
        items=[DomainItem(27), DomainItem(28)],
        interpretations=_interp_triad(
            [
                "Низкая потребность в особом признании, комфортно без повышенного внимания. Самооценка менее зависит от внешних оценок.",
                "Умеренная потребность в признании и одобрении. Чувствителен к вниманию, но способен сохранять баланс.",
                "Выраженная потребность в особом отношении и признании. Риск сильного расстройства при отсутствии похвалы и напряжения в отношениях.",
            ]
        ),
    ),
    DomainDefinition(
        ...
        title="Психопатия",
        visibility="internal",
        items=[DomainItem(29), DomainItem(30, True)],
        interpretations=_interp_triad(
            [
                "Низкая импульсивность и высокая сдержанность в риске. Принимает в расчёт последствия для других.",
                "Умеренная импульсивность; решения могут колебаться между риском и осторожностью. Вред другим иногда служит стоп‑фактором.",
                "Повышенная импульсивность и низкое сдерживание вреда другим при рисковых решениях. Риск резких поступков без учёта последствий.",
            ]
        ),
    ),
]
...
class HexacoEngine:
    ...
    def calculate(self, answers: Dict[int, int]) -> List[HexacoResult]:
        ...
        for domain in self.domains:
            ...
            band = self._resolve_band(domain.id, percent)
    ...
    def _resolve_band(self, domain_id: str, percent: float) -> Band:
        bands = BANDS_TRIAD if domain_id.startswith("dt_") else BANDS
        for band in bands:
