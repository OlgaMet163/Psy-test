...
## Работа с ботом
- Меню только inline: порядок кнопок — «Начать» → «Результаты» → «Перепройти» для каждого теста.
...
- Радар-диаграммы (HEXACO, Hogan, SVS) строятся на matplotlib; при сбоях отправлять текст без падений.
- Команда `teamswitch` (или текст) переводит в режим сотрудника, доступна только кнопка «Посмотреть участника».
- IM (Социально-одобряемые ответы) показываем только сотруднику; для участника IM скрыт.
